...
def get_price_delhi(location, sqft, bhk, bath):
    try:
...
def load_artifacts_delhi():
    print("Loading saved artfacts")
    global __data_columns
    global __locations
    
    with open(r"D:\RKT\Python files\my projects\bang_lore_final_final\Delhi_columns_rfr.json","r") as f:
        __data_columns=json.load(f)["data_columns"]
        __locations =__data_columns[3:]
        # __locations = [col.replace("location_", "") for col in __data_columns[3:]]
        
    global __model
    with open(r"D:\RKT\Python files\my projects\bang_lore_final_final\Delhi_rfr_model.pickle","rb") as f:
